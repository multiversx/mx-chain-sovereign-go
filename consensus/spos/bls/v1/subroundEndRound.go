...
func (sr *subroundEndRound) aggregateSigsAndHandleInvalidSigners(bitmap []byte) (*aggregatedSigsResult, error) {
	...
	err = sr.SigningHandler().Verify(sr.getMessageToVerifySigFunc(), bitmap, header.GetEpoch())
	if err != nil {
...
func (sr *subroundEndRound) verifyNodesOnAggSigFail() ([]string, error) {
	...
	for i, pk := range pubKeys {
		...
		isSuccessfull := true
		err = sr.SigningHandler().VerifySignatureShare(uint16(i), sigShare, msg, header.GetEpoch())
		if err != nil {
			isSuccessfull = false
