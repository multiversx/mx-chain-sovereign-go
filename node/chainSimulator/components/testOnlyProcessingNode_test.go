...
func createMockArgsTestOnlyProcessingNode(t *testing.T) ArgsTestOnlyProcessingNode {
	...
	return ArgsTestOnlyProcessingNode{
		...
		CreateIncomingHeaderSubscriber: func(config config.WebSocketConfig, dataPool dataRetriever.PoolsHolder, mainChainNotarizationStartRound map[string]config.MainChainNotarization, runTypeComponents mainFactory.RunTypeComponentsHolder) (process.IncomingHeaderSubscriber, error) {
